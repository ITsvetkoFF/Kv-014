...
import edu.softserve.zoo.persistence.specification.stubs.sql.GetAllEmployeesSQLStub;
import edu.softserve.zoo.persistence.specification.stubs.sql.GetEmployeeByIdSQLStub;
import edu.softserve.zoo.persistence.specification.stubs.sql.GetEmployeeByNameSQLStub;
...
public class SQLSpecificationTest {
    ...
    public void getAllTest() {
        int expectedCount = 3;
        List<Employee> employees = getList(repository.find(new GetAllEmployeesSQLStub()));
    ...
    public void getByIdCorrectId() {
        ...
        List<Employee> employees = getList(repository.find(new GetEmployeeByIdSQLStub(employeeId)));
        int actualCount = employees.size();
        assertEquals("Should collect all employees", expectedCount, actualCount);
    ...
    public void getByParameterCorrect() {
        ...
        List<Employee> employees = getList(
                repository.find(new GetEmployeeByNameSQLStub(employeeFirstName)));
        int actualCount = employees.size();
        assertEquals("Should collect all employees", expectedCount, actualCount);
        String actualEmployeeEmail = employees.get(0).getEmail();
        assertEquals("Emails should be the same", expectedEmployeeEmail, actualEmployeeEmail);
    ...
    public void getByParameterIncorrect() {
        ...
        int expectedCount = 0;
        List<Employee> employees = getList(repository.find(new GetEmployeeByNameSQLStub(employeeFirstName)));
        int actualCount = employees.size();
        assertEquals("Should collect all employees", expectedCount, actualCount);
    }
