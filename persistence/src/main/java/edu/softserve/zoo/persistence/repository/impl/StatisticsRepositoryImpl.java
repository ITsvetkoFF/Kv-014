...
/**
 * Created by Taras on 03.05.2016.
 */
@Repository
...
//        return (Float) sessionFactory.getCurrentSession().createSQLQuery("SELECT 1 - num/CAST(animals AS FLOAT) as result FROM (SELECT count(*) AS num FROM ZOO.TASKS WHERE TASK_TYPE_ID = 0 AND STATUS_ID IN (1,3)), (SELECT count(*) AS animals FROM ZOO.ANIMALS);").addScalar("result", FloatType.INSTANCE).uniqueResult();
    }
    @Override
...
//        final Collection<Object[]> list = sessionFactory.getCurrentSession().createSQLQuery("SELECT STATUS_ID as status, count(*) as num from ZOO.TASKS WHERE ASSIGNEE_ID = " + employee.getId() + " GROUP BY status").addScalar("status", IntegerType.INSTANCE).addScalar("num", IntegerType.INSTANCE).list();
...
//        final Collection<Object[]> list = sessionFactory.getCurrentSession().createSQLQuery("SELECT TASK_TYPE_ID as type, count(*) as num from ZOO.TASKS WHERE ASSIGNEE_ID = " + employee.getId() + " GROUP BY type").addScalar("type", IntegerType.INSTANCE).addScalar("num", IntegerType.INSTANCE).list();
...
//        final Object[] res = (Object[]) sessionFactory.getCurrentSession().createSQLQuery("SELECT * FROM" +
//                " (SELECT count(*) AS animals FROM ZOO.ANIMALS)," +
//                " (SELECT count(*) AS houses FROM ZOO.HOUSES)," +
//                " (SELECT count(*) AS employees FROM ZOO.EMPLOYEES)")
//                .addScalar("animals", IntegerType.INSTANCE).addScalar("houses", IntegerType.INSTANCE).addScalar("employees", IntegerType.INSTANCE).uniqueResult();
